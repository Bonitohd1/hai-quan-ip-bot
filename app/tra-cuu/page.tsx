          ...
          <div className="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden">
             ...
             <div className="w-full lg:w-1/2 border-b lg:border-b-0 lg:border-r border-slate-200 relative flex flex-col bg-[#0a192f] min-h-[280px] lg:min-h-0">
                ...
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10 flex-shrink-0">
                  ...
                  <FileText className="w-3.5 h-3.5 text-orange-400 ml-2"/>
                  ...
                  {doc.filename && (
                    <a href={`/documents/${doc.filename}`} target="_blank" rel="noopener noreferrer"
                      ...
                      <ExternalLink className="w-3 h-3"/> Mở tab
                    </a>
                  )}
                </div>

                {/* PDF via proxy iframe OR info card */}
                {doc.filename ? (
                  <iframe
                    src={`/api/pdf?file=${encodeURIComponent(doc.filename)}&folder=documents`}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-6 gap-4">
                    ...
                    <div className="text-center space-y-1">
                      ...
                      <p className="text-slate-500 text-xs">Chưa có file PDF đính kèm trong hệ thống.</p>
                    </div>
                    <div className="w-full max-w-xs bg-white/5 border border-white/10 rounded-xl p-4">
                ...
                {doc.relatedFiles && doc.relatedFiles.length > 0 && (
                  <div className="flex-shrink-0 px-4 py-2 bg-amber-500/10 border-t border-amber-500/20 flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-black text-amber-400 uppercase tracking-wider">Đính kèm:</span>
                    {doc.relatedFiles.map((f) => (
                      <a key={f} href={`/api/pdf?file=${encodeURIComponent(f)}&folder=documents`} target="_blank" rel="noopener noreferrer"
                ...
                {/* Bottom download bar */}
                <div className="flex-shrink-0 px-4 py-2.5 border-t border-white/10 flex items-center gap-2">
                  <button onClick={handleExportPDF}
                    className="flex-1 flex items-center justify-center gap-2 py-2 bg-white/10 hover:bg-white/20 text-white font-bold text-xs rounded-lg transition-colors">
                    <Download className="w-3.5 h-3.5"/> Tải xuống {doc.relatedFiles?.length ? 'tất cả' : 'PDF'}
                  </button>
                  {doc.filename && (
                    <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-medium">
                      <span className={`w-1.5 h-1.5 rounded-full ${doc.type === 'Cấp mới' ? 'bg-blue-400' : 'bg-emerald-400'}`}/>
